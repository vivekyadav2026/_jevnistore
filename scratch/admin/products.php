        ...
        <tbody>
            ...
            if ($prods->num_rows > 0) {
                while ($p = $prods->fetch_assoc()) {
                    $img = $p['image'] ? '/assets/' . htmlspecialchars($p['image']) : '/assets/product_pants.png';
...
function openEditModal(product, gallery) {
    ...
    if (product.image) {
        curImg.innerHTML = `<img src="/assets/${product.image}" style="max-width:80px; max-height:80px; border-radius:4px; border:1px solid #333;"> <span style="font-size:12px; color:#666;">(${product.image})</span>`;
    } else {
    ...
    if (product.image2) {
        curImg2.innerHTML = `<img src="/assets/${product.image2}" style="max-width:80px; max-height:80px; border-radius:4px; border:1px solid #333;"> <span style="font-size:12px; color:#666;">(${product.image2})</span>`;
    } else {
    ...
    if (gallery && gallery.length > 0) {
        gallery.forEach(img => {
            ...
            item.style.height = '60px';
            item.innerHTML = `
                <img src="/assets/${img.image_path}" style="width:100%; height:100%; object-fit:cover; border-radius:4px; border:1px solid #333;">
